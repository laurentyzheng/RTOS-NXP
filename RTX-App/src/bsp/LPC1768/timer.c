...
volatile TM_TICK t_last_interrupt = {0, 0};
...
void TIMER0_IRQHandler(void)
{
    /* ack inttrupt, see section  21.6.1 on pg 493 of LPC17XX_UM */
		LPC_TIM0->IR = BIT(0); // acknowledge
	
    TM_TICK right_now = {0, 0};
    get_tick(&right_now, TIMER1);
		
    
    U32 us_passed = 0;
    us_passed = (right_now.tc - t_last_interrupt.tc) * SEC_IN_uS;
    us_passed += (right_now.pc - t_last_interrupt.pc) / PC_PER_uS;
	U32 tick_passed = us_passed / RTX_TICK_SIZE;
		
//		if (g_timer_count % 10 == 0){
//			printf("us:%d\r\n",us_passed);
//		}
		
		if (tick_passed == 0 || tick_passed > 100){
			// > 100 is usually garbage
			tick_passed = 1;
		}
		
    g_timer_count += tick_passed;

    //updating deadlines
		if (gp_current_task->prio == PRIO_RT){
			gp_current_task->deadline -= tick_passed;
		}

		if (susQueue.tail != NULL){
			susQueue.tail->deadline -= tick_passed;
		}

		if (rQueues->tail != NULL) {
			rQueues->tail->deadline -= tick_passed;
		}

    enqueue_front (gp_current_task, rQueues);
		
    while (susQueue.tail && susQueue.tail->deadline <= 0)
    {
        TCB* released = dequeue(&susQueue);
#ifdef DEBUG_0
				printf("releasing tid = %d. \n\r", released->tid);
#endif /* DEBUG_0 */
				released->deadline = timeval_to_tick(released->period);
        insert_rt_queue(released, rQueues);
    }
    
    t_last_interrupt = right_now;
    k_tsk_run_new();
}
