...
uint8_t g_buffer[UART_MBX_SIZE];
...
static U8 msg_iterator = 0;
static RTX_MSG_HDR * recv_header;
...
void UART0_IRQHandler(void)
{
    ...
    if (IIR_IntId & IIR_RDA) { /* Receive Data Avaialbe */
        ...
        if (!g_tx_irq) {
            g_char_in = char_in;
            char *buf1 = k_mpool_alloc( MPID_IRAM2 , MSG_HDR_SIZE + sizeof(char));
            RTX_MSG_HDR * key_in_header = (RTX_MSG_HDR*) buf1;
            key_in_header->length = MSG_HDR_SIZE + sizeof(char);
            key_in_header->sender_tid = TID_UART;
            key_in_header->type = KEY_IN;
            *(buf1 + sizeof(RTX_MSG_HDR)) = g_char_in;
            int result = irq_send_to_KCD(buf1);
			if (result != RTX_OK){
						printf("uart failed to send to KCD!!!!!");
			}
            k_mpool_dealloc( MPID_IRAM2, buf1 );
#ifdef DEBUG_0
...
//						pUart->THR = '>';
        ...
        uint8_t char_out;
				...
				int result = (irq_receive(&g_buffer) == RTX_OK);
			
				if (result){
					recv_header = (RTX_MSG_HDR *)gp_buffer;
					gp_buffer += sizeof(RTX_MSG_HDR); //shift buffer to the actual message portion
				}


        /* THRE Interrupt, transmit holding register becomes empty */
        if ( (msg_iterator < recv_header->length - MSG_HDR_SIZE) && (*gp_buffer != '\0') ) {  // not end of the string yet
            char_out = *gp_buffer;
#ifdef DEBUG_1
            ...
            if (recv_header->sender_tid != TID_WCLCK){
                pUart->THR = '\r';
                pUart->THR = '\n';
                pUart->THR = '>';
                pUart->THR = '>';
            ...
            gp_buffer = g_buffer;    // reset the buffer  
						//receive from normal user tasks
        }          
